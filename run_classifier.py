...
flags.DEFINE_string(
    ...
    "The path for pre-trained model.",
)
# 指定输入数据路径
flags.DEFINE_string(
...
# 指定预训练模型的名称，大小写均可，默认为“albert”
flags.DEFINE_string(
    "model_name", 'bert',
    "The name of pre-trained model, eg:'bert','albert'",
)

flags.DEFINE_string(
    "data_dir", './data/Dispose_folder/',
    "The input datadir.",
)
...
flags.DEFINE_string("task_name", 'classifier', "The name of the task to train.")
...
class classifierProcessor(DataProcessor):
  ...
  def get_labels(self):
    ...
    df.columns=['0','1','2']
    self.label = df['2'].to_list()
    self.label = sorted(self.label)
    self.label = [str(i) for i in self.label]
    # return [str(x) for x in range(21)]
    #print(dictR)
    return self.label
...
def save_PBmodel(num_labels):
  ...
  try:
      ...
      pb_file = os.path.join(FLAGS.output_dir, 'Bert_Textclassification_model.pb')
      graph = tf.Graph()
      with graph.as_default():
          ...
          loss, per_example_loss, logits, probabilities = create_model(
              ...
              input_ids=input_ids, input_mask=input_mask, segment_ids=None, labels=None, num_labels=num_labels,use_one_hot_embeddings=None)
...
def main(_):
  ...
  processors = {
      "classifier": classifierProcessor, #2020/3/6 add by xmxoxo
  }
  ...
  if FLAGS.do_eval:
    ...
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      ...
      for key in sorted(result.keys()):
        ...
        writer.write("%s = %s\n" % (key, str(result[key])))
    save_PBmodel(num_labels=len(label_list))
...
if __name__ == "__main__":
  ...
  outpath = arg_dic["data_dir"]
  if  not os.path.exists(outpath):
      os.makedirs(outpath)
  print(sourcefile)
  print(outpath)
  if not os.listdir(sourcefile):
      print("请先将标注好的数据放入{}此文件夹下".format(sourcefile))
  else:
      output = data_analye.dataPro(sourcefile,outpath)
      #输出数据类别分布情况
      data_analye.pd_datCheck(output)
      #划分数据集 8：2：10
      fnout = data_analye.CreateCategoryTrain(outpath)
      print(fnout)
      FLAGS.data_dir = fnout
      if  not os.listdir(fnout):
          print("请重新生成训练数据文件")
      else:
          os.environ["CUDA_VISIBLE_DEVICES"] = FLAGS.GPU
          flags.mark_flag_as_required("model_path")
          # 根据模型名称自动处理预训练模型文件名
          if not FLAGS.model_name.upper() in ['BERT','ALBERT'] :
            raise ValueError("Please set model_name,eg: 'bert' or 'albert'.")

          if FLAGS.model_name.upper()=='BERT':
            FLAGS.bert_config_file = os.path.join(FLAGS.model_path, 'bert_config.json')
            FLAGS.init_checkpoint = os.path.join(FLAGS.model_path, 'bert_model.ckpt')
            FLAGS.vocab_file = os.path.join(FLAGS.model_path, 'vocab.txt')
          ...
          if FLAGS.model_name.upper()=='ALBERT':
            FLAGS.bert_config_file = os.path.join(FLAGS.model_path, 'albert_config_tiny.json')
            FLAGS.init_checkpoint = os.path.join(FLAGS.model_path, 'albert_model.ckpt')
            FLAGS.vocab_file = os.path.join(FLAGS.model_path, 'vocab_chinese.txt')
